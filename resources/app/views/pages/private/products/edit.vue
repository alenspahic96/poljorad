...
const loadProduct = async () => {
  ...
  category_id.value = res.data.category_id ?? res.data.category?.id ?? null
  preview.value = res.data.image_url ?? null
}
...
<template>
  <div class="p-4">
    ...
    <div class="mb-3">
      ...
      <input v-model="description" class="input input-bordered w-full" />
    </div>
    ...
    <div class="mb-4">
      ...
      <div v-if="preview" class="mt-4">
        <div class="relative w-40 group cursor-pointer" @click="openFile">
          <img
            :src="preview"
          ...
          <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center rounded-xl">
            <span class="text-white text-sm">Promijeni</span>
          </div>
        </div>
      </div>
    </div>

    <button class="btn btn-primary mt-4 flex items-center justify-center" @click="submit">
      Save changes
    </button>
  </div>
</template>